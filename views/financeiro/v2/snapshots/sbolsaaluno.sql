...
-- MATERIALIZED VIEW (refresh: REFRESH MATERIALIZED VIEW [CONCURRENTLY] export_v2.sbolsaaluno)
-- Recriar exige: DROP MATERIALIZED VIEW export_v2.sbolsaaluno; (+ reindexar UNIQUE INDEX)
CREATE MATERIALIZED VIEW export_v2.sbolsaaluno AS
WITH alunos AS (
         ...
         SELECT bd.calendario_academico,
            ...
            bd.item_descricao AS servico_original,
                CASE
                    WHEN bd.item_descricao::text ~* '1[^[:space:]]{0,3}\s*(MENS|PARC)'::text AND bd.item_descricao::text !~* '^\s*(MENS|PARC)'::text THEN '1ª mensalidade'::text
                    WHEN bd.item_descricao::text ~~* '%ANUID%'::text THEN 'Mensalidade'::text
                    WHEN bd.item_descricao::text ~~* '%MENS%'::text THEN 'Mensalidade'::text
                    WHEN bd.item_descricao::text ~~* '%ALIM%'::text THEN 'Alimentação'::text
                    WHEN bd.item_descricao::text ~~* '%MAT%DIDAT%'::text OR bd.item_descricao::text ~~* '%MDIDAT%'::text OR bd.item_descricao::text ~~* '%MDIAT%'::text OR bd.item_descricao::text ~* '^\s*MD\s'::text OR bd.item_descricao::text ~~* '%MATERIAIS%'::text THEN 'Material Didático'::text
                    ELSE TRIM(BOTH FROM bd.item_descricao)
                END AS servico_nome,
                CASE
        ...
        UNION ALL
         ...
         SELECT b.calendario_academico,
          ...
          WHERE (EXISTS ( SELECT 1
                   FROM export_v2.sservico s
                  WHERE s."NOME"::text = b.servico_nome)) AND (EXISTS ( SELECT 1
         ...
         SELECT a.codfilial,
          ...
          GROUP BY a.codfilial, a."CODCURSO", a."CODHABILITACAO", a."CODGRADE", a."TURNO", a.ra, a.academic_calendar, ct.id_contract, b.servico_nome, b.nome_bolsa, b.forma
        )
 SELECT 1 AS "CODCOLIGADA",
    ...
    "left"(servico_nome, 60)::character varying(60) AS "SERVICO",
...
-- Indices existentes:
-- CREATE UNIQUE INDEX sbolsaaluno_uk ON export_v2.sbolsaaluno USING btree ("CODCOLIGADA", "RA", "CODPERLET", "CODCONTRATO", "NOMEBOLSA", "SERVICO", "PARCELAINICIAL");
